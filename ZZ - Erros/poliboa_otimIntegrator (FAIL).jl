...
function map_cells(bp :: BasinProblem, cell_range :: UnitRange{Int64}, p) :: BasinResult
    # Cria a grade que será populada na função
    grid = zeros(Int64, Tuple(x for x in bp.region.elements))

    # Cria a lista de atratores para preenchimento
    attractors = []
...
function search_for_attractor_discrete!(bp :: BasinProblem, grid, transition_map :: Dict{Int64, Int64}, 
    ...
    # Follow the transition map to build trajectory
    cycles = 0
    while cycles < bp.maximum_cycles
        # Check if this cell was already assigned to an attractor
        if grid[current_cell] != 0
            # Bumped into a known attractor
            attr_number = grid[current_cell]
            break
        end
        
        # Add current cell to trajectory
        T[T_len += 1] = current_cell
        
        # Check if we've seen this cell before in current trajectory
        occurrences = count(c -> c == current_cell, @view T[1:(T_len - 1)])
        
        if occurrences > 0
            # Found a cycle! Mark all cells from first occurrence as attractor
            first_occurrence = findfirst(c -> c == current_cell, @view T[1:(T_len - 1)])
            
            # The attractor consists of cells from first occurrence to current position
        ...
        # Get next cell from transition map
        if !haskey(transition_map, current_cell)
            # This shouldn't happen if transition map is complete, but handle gracefully
            attr_number = -1
            break
        end
    ...
    # Check if we hit maximum cycles without finding attractor
    if cycles == bp.maximum_cycles && !found_new_attractor
        # Assume quasi-periodic behavior
        found_new_attractor = true
        attr_type = quasi_periodic
        attr_number = next_attr_num
        # Use second half of trajectory as attractor representation
    ...
    # Mark all cells in trajectory with attractor number
